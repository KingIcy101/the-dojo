  ...
  for (let i = 0; i < maxRetries; i++) {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: { Authorization: `Bearer ${process.env.OPENROUTER_API_KEY}`,
        'HTTP-Referer': 'https://inthepast.ai', 'Content-Type': 'application/json' },
...
| Model | Use Case |
|-------|----------|
| `anthropic/claude-sonnet-4-6` | Judgment, writing, reasoning |
| `google/gemini-2.5-pro-preview` | Large context, long codebases |
| `openai/gpt-4o` | Voice/Echo/Dex agent work |
| `openai/gpt-4o-mini` | High-volume cheap tasks |

## Gotchas
- Missing `HTTP-Referer` → false 403 — always include it
